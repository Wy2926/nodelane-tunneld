...
const controlRunColumns = `id, route_id, started_via, status, desired_state, host(request_ip), host(connected_ip), created_at, connected_at, last_heartbeat_at, stop_requested_at, stopped_at, connect_deadline_at, lease_expires_at, stop_reason`
...
func scanControlRun(row controlScanner) (domain.Run, error) {
	...
	if err := row.Scan(&run.ID, &run.RouteID, &run.StartedVia, &run.Status, &run.DesiredState,
		...
		&run.StopRequestedAt, &run.StoppedAt, &run.ConnectDeadlineAt, &run.LeaseExpiresAt, &stopReason); err != nil {
