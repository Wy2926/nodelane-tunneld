...
- 文档职责：记录已确认需求和编码约束；实施进度、未完成项与验证证据统一见[开发交接](../../HANDOFF.md)，不在此重复维护阶段状态。
...
- **MIG-03**：本轮只提供 Goose 最终第一版结构的全新初始化；重复启动可检查同一结构，不实现旧数据库升级路径。
...
来源 IP 属于传输元数据，不计入请求摘要。相同认证与幂等请求可以在来源 IP 改变后恢复同一结果，但保留首次提交的请求 IP 和原始截止时间；每次请求的来源有效性、当前认证及归属检查仍正常执行。
...
- 2026-09-06 确认的回收标准：业务运行 ID、路由 ID 和 URL 不变，每次 frps Login 使用独立原生会话 ID。权威存储原子记录是否批准过 NewProxy；从未批准且授权终止的运行直接按所属资源 CAS 回收。已批准过的运行先取得当前原生客户端已断开的证据，再核对代理入口离线且连接数为零，或在注册处理已结束的前提下确认入口不存在。证据不足保持占用，不仅凭 404 或期限推断释放。
- “允许入口复用”不表示停止前所有旧数据流已结束；正常官方客户端仍须满足 15 秒停止验收。原生会话与业务 ID 分离防止旧登录失败的迟到状态覆盖新会话，不改变业务资源归属。
...
以下是另行获得目标环境、备份和执行权限确认后才适用的操作约束，不是当前交接任务的执行授权。生产切换是人工维护窗口，不嵌入普通容器启动命令：
...
当前代码入口、剩余缺口及复现命令见[开发交接](../../HANDOFF.md)。已实现的阶段不重新规划或重做；后续按实际缺口推进代码、测试和审查，不生成重复阶段总结。

真实身份连接器的验证依赖已授权部署输入，不能用 OIDC 测试替身代替真实 Resend、Google、SSO 或 Device Flow 验收。生产删库重建仍以 staging 完整验收、备份/回滚演练和单独授权为前提。
...
- Web 国际化统一使用既有 `web/src/i18n/index.ts`、`types.ts` 和 12 个语言包；控制台及错误文案不得另建词典、注册表或兼容转发入口。新增字段同时覆盖 12 种语言及 Arabic RTL，保持键盘焦点、移动端和 reduced-motion。详情路径的语言切换不能丢失资源 ID。
- 样式以当前 `web/src/styles/design-system.css` 和共享组件为准，文档中的旧页面快照或尺寸描述不覆盖源码基准。网站与控制台复用页头、品牌、语言选择器及控件样式，不新建并行设计框架；业务布局按页面职责组织。跨仓库共享样式明确来源与同步验证，不声称已经字节一致而不检查。
...
- 按实际变更范围进行独立规格审查和代码审查，问题修正后再推进；进展和剩余缺口更新到同一交接记录，不以重复计划或总结替代开发与测试。
