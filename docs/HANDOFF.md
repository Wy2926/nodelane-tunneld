...
| 仓库 | 已验证的功能基准 | 当前边界 |
| --- | --- | --- |
| `nodelane-tunneld` | 功能基准 `c876e4e`，主要功能提交 `172038e`；最新发布源码 `b1efa68` | `0.5.1` 镜像和 `stable` 已发布，内置 `nt 0.5.0` 不变；生产未部署，仍有下述缺口 |
| `auth` | 原功能基准 `a22e375`；最新发布源码 `f270b06` | 根路径、协议/联系入口及品牌邮件本地已应用；GitHub 已推送，双架构 `.2` 镜像已发布，生产未部署 |
| `nodelane-www` | 功能基准 `4c2f640`；当前 `main`：`813e84e` | 12 语言政策页面及统一联系邮箱有未提交修改，公共页头控制台按钮已移除；未推送或部署 |
...
5. **0.5.1 Git 与镜像发布已完成。** 服务端支持公网与 frps 内部端口分离，版本镜像和 `stable` 均已回读确认；内置 `nt` 保持原 `0.5.0` 包字节，没有额外触发 `nt-v*` GitHub 客户端 Release。本地部署包和 FRP 证书已生成，正式 Logto 三项绑定及生产新库初始化仍待用户完成；没有执行生产部署、远端密钥轮换、提供方修改或真实数据库清理。详细证据见下方补丁节。

不要将剩余工作概括成“仅差上线配置”，也不要因全部现有测试通过而宣称全部需求闭环。

## Auth 邮件与 1.43.0-nodelane.2 发布

2026-09-06 用户授权选择一个统一接收联系的邮箱、移除官网控制台按钮、设计品牌验证码邮件，并推送 `https://github.com/Wy2926/nodelane-auth.git` 与私有 Docker。用户不要求继续处理极窄分辨率；没有授权修改 Logto 上游、生产租户或 Google 提供方。

- **联系与官网。** 统一采用 `contact@nodelane.net`，所有语言共用一个 ASCII 地址；主站页脚、隐私政策、条款和 Auth 支持链接均指向该地址。官网的公共页头控制台按钮和闲置文案已移除。主站修改仍在独立 `nodelane-www` 工作区，未推送到其他仓库，也未随 Auth 镜像部署。用户声明域名可收件，实际投递及单个地址路由未发送测试邮件验证。
- **原生品牌邮件。** [email-templates.mjs](../../auth/scripts/email-templates.mjs) 生成 Register / SignIn / ForgotPassword / Generic 四种中英双语 HTML 模板，复用官网 PNG、深色品牌页头、薄荷绿强调和浅色正文。验证码只出现于正文，主题和 preheader 不含验证码；不加入跟踪、脚本或虚构有效期。发件人保持 `NodeLane <auth@nodelane.net>`，回复地址为统一联系邮箱。ForgotPassword 模板为原生连接器必需，并未开放密码登录。原生 SMTP 每个模板只提供一种正文类型，本次不自建双 MIME 或邮件协议。
- **应用工具与本地状态。** [apply-email-templates.mjs](../../auth/scripts/apply-email-templates.mjs) 复用现有精确 issuer/TLS/回环保护和私密输入检查，默认只读计划，`--apply` 后回读，`--check` 检测漂移。只更新模板和 reply-to，保留完整 SMTP 凭据、发件人和其他设置；外来模板需显式替换参数，原生按语言邮件模板有任何覆盖时先拒绝，不删除其配置。写前复查不能替代原生 API 不具备的原子比较更新，操作期间避免并发改配置。现有本地租户已应用，两个检查工具均无漂移；未发送验证码或测试邮件。
- **Git 和私有镜像。** 源码提交 `f270b06ad99da46d4272fd5ccc99111fcbeb9185` 已推送并回读为目标 GitHub 的 `main`，后续发布文档提交会推进 HEAD。`docker.nodelane.net/nodelane/auth:1.43.0-nodelane.2` 已由 Docker / regctl 独立回读为 `sha256:b10325f1485b6d63628624f28f34f1d22686c4e5a781b443526eb0926f4e5cb4`，包含 amd64、arm64 和两份 SLSA 构建证明。两架构保留七个原版层，仅增加两个 COPY 层；六个 JS 与 CSS 和发布提交字节一致，仅镜像 title/version/revision 标签标识 NodeLane 发布。未覆盖 `.1`，未创建 `stable`，未升级上游或执行生产部署。正式使用和租户配置命令见 [Auth README 第 7.1 / 15 节](../../auth/README.md)。
- **验证与剩余项。** Auth 全套 351 项中 346 通过、5 项显式平台或 opt-in 跳过；原生 SMTP 校验/替换使用内存传输替身，没有网络发信。Go 密钥生成器测试、本地 TLS/Discovery/JWKS/Native 配置与 Tunnel PKCE 跳转通过。主站源码测试 6 项、构建测试 26 项通过，构建 36 页无诊断；真实浏览器确认官网控制台链接为零，官网及 Auth 邮件链接一致。已按发布摘要拉取并在网络隔离、只读、非 root 容器中运行工具，amd64 与模拟 arm64 的主题 SHA-256 为 `371b1be9fb0b02dec64086643c9e76a53860e970fed1c1ee272f02398c05d3ea`，模板集合为 `2f352684b963c4780541939ca90c8e7c9a3d8938ae42a832326fa98817ade203`。51 个 Git 候选文件对 17 个真实秘密值扫描无匹配，历史源码检查无私钥/Token。合成 HTML 预览位于 Auth `.reports/email-preview-20260906`，不进 Git/镜像；实际收件、Outlook/Gmail 展示和像素目视未验收。社交绑定冲突 toast 的上游缺陷仍按下节待确认，不能因本次发布而宣称已修复。

## Auth 页面及 OAuth 公开页面补充

本节保留上一轮本地实现及当时的未提交状态；后续联系邮箱和正式 Git/镜像发布以上节为准。
...
- 发布源码 `b1efa6831e7e5bf866dacb620890bf22fe910ecb` 已推送并回读；`docker.nodelane.net/nodelane/tunneld:0.5.1` 和 `:stable` 均为 `sha256:cc1ed4eb48e2a0209ebf43d6c6045a5dc993db2f0b05bac9011e49613d985f08`，包含 Linux amd64/arm64 及构建证明。镜像只从固定 Git 快照构建，不包含本地私密配置或未跟踪的 `nt`。
- 本轮真实隔离 PostgreSQL/Redis 下 Go 全套 30 包通过、1942 项测试/子测试通过、2 项预期辅助/预览跳过；`go vet ./...`、双程序构建、格式和部署检查通过。真实 frpc 经不同端口 TCP 转发器连接原版 frps，HTTP 后端、停止与回收均通过；匿名、原生流量及 Compose 端口组合用例也实际通过。Web 源测试 29 通过/1 项 POSIX 跳过、构建测试 22 通过、嵌入一致性 1 通过。
- Registry 拉取后的 19 项隔离镜像检查通过，正常服务启动接受公网 `7001` / 内部 `7000`。四个平台旧 `nt 0.5.0` 包的 SHA-256 和大小不变，`stable.txt` 仍为 `0.5.0`；24 个公开前端文件逐字节一致，`run.sh` 为 5530 字节且 `CR=0`。amd64 与模拟 Linux arm64 的服务端/客户端版本运行通过，不冒充 ARM64 实机验收；临时容器、网络和临时密钥均已清理。
- 本地用户上传包固定 `0.5.1` 摘要，两服务没有 Compose 启停依赖；FRP 证书覆盖 `frp.nodelane.net`、到期日为 2027-09-06，离线 CA 到期日为 2031-09-06。证书链、SNI、私钥匹配、原版 frps 配置语法、Compose 挂载/端口、七个独立密钥与本地 ACL 均已校验。实际上传包检查会因三个空的正式 OIDC 绑定而拒绝就绪，不能将配置文件已生成称为生产可用。

下节保留 `0.5.0` 当时的发布证据和通用部署步骤；当前 `stable` 以本节为准，`0.5.1` 仍未在生产部署。
